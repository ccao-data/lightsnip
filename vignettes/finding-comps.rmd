...
Now that we've seen how LightGBM trees are structured, we can use them to create a _similarity score_. This score is simply the percentage of shared leaf nodes between a target and comparable property.
...
The observations with a high percentage of shared leaf nodes—that is, the observations with the highest similarity scores—should be most comparable to the target property.
...
ames_final_matrix_fmt <- ames_final_matrix %>%
  bind_cols(
    tibble(ID = leaf_4_ids),
    .
  ) %>%
  ...
  mutate(
    across(starts_with("Tree "), as.integer, .names = "score_{.col}"),
    across(starts_with("score_"), ~ .x / 10),
    `Sim. Score` = rowSums(across(starts_with("score_"))),
    `Sim. Score` = scales::percent(`Sim. Score`, accuracy = 1)
  ) %>%
  # Drop internal representation of tree matches now that we have scores
